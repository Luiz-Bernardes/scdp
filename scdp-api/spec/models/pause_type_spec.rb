require "rails_helper"

RSpec.describe PauseType, type: :model do
  subject { build(:pause_type) }

  describe "associations" do
    it { should belong_to(:team) }
    it { should have_many(:pauses).dependent(:destroy) }
    it { should have_many(:pause_queues).dependent(:destroy) }
  end

  describe "validations" do
    it { should validate_presence_of(:name) }
    it do
      should validate_numericality_of(:max_concurrent)
        .is_greater_than(0)
    end

    it do
      create(:pause_type)
      should validate_uniqueness_of(:name).scoped_to(:team_id)
    end

    it "requires max_duration_minutes for timed pauses" do
      pause_type = build(
        :pause_type,
        has_time_limit: true,
        max_duration_minutes: nil
      )

      expect(pause_type).not_to be_valid
      expect(
        pause_type.errors[:max_duration_minutes]
      ).to include("must be present for timed pauses")
    end

    it "does not require max_duration_minutes for non timed pauses" do
      pause_type = build(
        :pause_type,
        has_time_limit: false,
        max_duration_minutes: nil
      )

      expect(pause_type).to be_valid
    end
  end

  describe "scopes" do
    let!(:active_pause_type) { create(:pause_type, active: true) }
    let!(:inactive_pause_type) { create(:pause_type, active: false) }

    it "returns only active pause types" do
      expect(PauseType.active).to include(active_pause_type)
      expect(PauseType.active).not_to include(inactive_pause_type)
    end
  end
end